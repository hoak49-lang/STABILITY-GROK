...
Aligned with ICH Q1A(R2) / ICH Q1E (supportive analysis only)

Supports single- and multi-attribute (assay + impurity + …) analysis.
...
with st.sidebar:
    ...
    st.caption("Shelf-life / Stability · ICH Q1E")
    st.markdown("---")
...
if step.startswith("Giới thiệu"):
    brand_header("Giới thiệu / About & Methods", "ICH Q1A(R2) · ICH Q1E · supportive analysis")
...
5. **Arrhenius (SUPPORTIVE):** fit ln(k)=ln(A)−Ea/(R·T) từ bảng T+k hoặc
...
if step.startswith("Arrhenius"):
    ...
    callout(
        "SUPPORTIVE / HỖ TRỢ ONLY — Không thay thế nghiên cứu ổn định dài hạn ICH Q1A(R2) / Q1E. "
...
# ===========================================================================
brand_header(
    "Phân tích ổn định / Stability & Shelf-Life",
    "ICH Q1E-aligned · multi-attribute · one-sided CI of the mean vs specification",
)
...
if step.startswith("1"):
    ...
    if df is not None and len(df) > 0:
        ...
        callout(
            "Tiếp theo: chuyển sang <b>2 · Mô hình / Model</b> để thiết lập spec, direction, transform "
...
# ===========================================================================
    ...
    section_header("2", "Mô hình", "Model settings")
    df = st.session_state["df"]
    if df is None or len(df) == 0:
        empty_state(
            "Chưa có dữ liệu",
            "Go to step 1 · Dữ liệu and load a sample, upload, or enter data.",
            "📋",
        )
        st.stop()
    ...
    callout(
        "Tiếp theo: <b>3 · Gộp lô / Pooling</b> (ANCOVA) nếu có nhiều lô, "
...
# ===========================================================================
    ...
    section_header("3", "Gộp lô", "Pooling / ANCOVA")
    df = st.session_state["df"]
    if df is None or len(df) == 0:
        empty_state("Chưa có dữ liệu", "Go to step 1 · Dữ liệu first.", "📋")
        st.stop()
...
"""
    ...
    callout("Chuyển sang <b>4 · Kết quả / Results</b> và nhấn <b>Chạy phân tích</b>.", kind="info")
...
# ===========================================================================
    ...
    section_header("4", "Kết quả", "Results")
    df = st.session_state["df"]
    if df is None or len(df) == 0:
        empty_state("Chưa có dữ liệu", "Complete step 1 · Dữ liệu before running analysis.", "📋")
        st.stop()
    ...
    callout("Xuất báo cáo HTML/PDF ở bước <b>5 · Báo cáo / Report</b>.", kind="info")
...
# ===========================================================================
    ...
    section_header("5", "Báo cáo", "Report export")
    analysis = st.session_state.get("analysis")
    if analysis is None:
        empty_state(
            "Chưa có kết quả để xuất",
            "Run analysis in step 4 · Kết quả first.",
            "📄",
        )
        st.stop()
    ...
    report_notes = (
        ...
        "confidence bound of the mean (ICH Q1E). Supportive analysis only. "
    ...
    html = build_html_report(
        ...
        shelf_html=f"<p><b>{ma.message}</b></p>",
        figures=all_figures if ma.n_attributes == 1 else [],
        notes=report_notes,
        attribute_sections=attr_sections if ma.n_attributes > 1 else None,
